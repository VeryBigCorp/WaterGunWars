<?php
    ...
    if($_POST['a'] == "login"){
        ...
        $alive = mysql_query("SELECT id FROM agent WHERE kia=false AND admin=false");
        $perc = 100 * mysql_num_rows($alive) / (mysql_num_rows($r) -2);
        $text = mysql_num_rows($alive) . " out of " . (mysql_num_rows($r)-2);
        
        echo "<br/>List of players ($perc% alive [$text]): <br/>";
        ...
        echo "Distributing...";
        dist_targets();
        mysql_query("UPDATE agent SET last_kill=NOW()");
        echo "Distributed.";
        ...
        mysql_query("DELETE FROM notif WHERE user='".get_killer($_SESSION['usr'])."'");
        $target = get_target($_SESSION['usr']);
        $killer = get_killer($_SESSION['usr']);
        mysql_query("UPDATE agent SET last_kill=NOW(), target='" . $target ."' WHERE code_name='" . $killer . "'");
        mysql_query("UPDATE agent SET kia=true, dispute=false, target='', prompt=false, killer='". $killer ."' WHERE code_name='".$_SESSION['usr']."'");
        ...
        while($row = mysql_fetch_assoc($q)){
            if($row['type'] == "DISP"){
                echo "<br/>&nbsp;&nbsp;&nbsp;&bull; " . $row['text']." <a style='text-decoration:underline;' onclick=\"disp(".$row['id'].",true, this);\">Killer Wins</a> or <a style='text-decoration:underline;' onclick=\"disp(".$row['id'].",false, this);\">Victim wins</a>";
            } else if($row['type'] == "CONF") {
                echo "<br/>&nbsp;&nbsp;&nbsp;&bull; " . $row['text']." <a style='text-decoration:underline;' onclick=\"con(".$row['id'].",true, this);\">Confirm kill</a> or <a style='text-decoration:underline;' onclick=\"con(".$row['id'].",false, this);\">Resolve</a>";
            } else {
                echo "<br/>&nbsp;&nbsp;&nbsp;&bull;<a onclick=\"notif(".$row['id'].",'".$row['type']."', this);\">".$row['text']."</a>";
            }
        }
        ...
        if($q['type'] == "CONF"){
            if($_POST['yes'] == true){
                ...
                $target = get_target($q['user']);
                $usr = $q['user'];
                mysql_query("UPDATE agent SET last_kill=NOW(), target='" . get_target($target) ."' WHERE code_name='" .$usr . "'");
                mysql_query("UPDATE agent SET kia=true, target='', prompt=false, killer='".$q['user']."' WHERE code_name='".$target."'");
                echo "Confirmed.";
                send_sms(get_tel($usr), get_carrier($usr),"You've been assigned a new target, " . get_real(get_target($usr)));
            } else {
                mysql_query("UPDATE agent SET prompt=false WHERE code_name='". get_target($q['user']) . "'");
            }
        } else if($q['type'] == "DISP"){
            if($_POST['yes'] == "true"){
                ...
                mysql_query("UPDATE agent SET last_kill=NOW(), target='" . get_target($q['user']). "' WHERE code_name='" .get_killer($q['user']) . "'");
                mysql_query(sprintf("UPDATE agent SET prompt=false, kia=true, target='', killer='%s' WHERE code_name='%s'", mysql_real_escape_string($killer), mysql_real_escape_string($q['user'])));
                mysql_query("UPDATE agent SET dispute=false WHERE code_name='".$killer."'");
                
                mysql_query("delete from notif where user='" . $q['user'] . " AND type='DISP'");
            } else {
                
                $killer = get_killer($q['user']);
                mysql_query(sprintf("UPDATE agent SET dispute=false WHERE code_name='%s'", mysql_real_escape_string($killer)));
                mysql_query(sprintf("UPDATE agent SET prompt=false WHERE code_name='%s'", mysql_real_escape_string($q['user'])));
                ...
                send_sms(get_tel($killer), get_carrier($killer),"The moderator sided with your target in the decision. You must attempt again.");
                mysql_query("delete from notif where user='" . $q['user'] . " AND type='DISP'");
            }
        }
        ...
        send_sms(get_tel(get_killer($_POST['player'])), get_carrier(get_killer($_POST['player'])),"Your target has killed you in self-defense.");
        $killer = get_killer(get_killer($_POST['player']));
        mysql_query("UPDATE agent SET kia=true, target='' WHERE code_name='" . get_killer($_POST['player']) . "'");
        mysql_query("UPDATE agent SET last_kill=NOW(), target='" . $_POST['player'] . "' WHERE code_name='" . $killer . "'"); 
        send_sms(get_tel($killer), get_carrier($killer),"Your target was killed by their target. You've been reassigned to " . get_real(get_target($killer)));
    ...
    } else if($_POST['a'] == "percent"){
    	$r = mysql_query("SELECT * FROM agent");
        
        $alive = mysql_query("SELECT id FROM agent WHERE kia=false AND admin=false");
        $perc = 100 * mysql_num_rows($alive) / (mysql_num_rows($r) -2);
		echo "$perc% of the agents are still alive. (" . mysql_num_rows($alive) . " people)";
    }
    mysql_close();
    ...
    function dist_targets(){
        ...
        while($r = mysql_fetch_assoc($q)){
            if($r['kia'] == true) {
                mysql_query("UPDATE agent SET target='' WHERE code_name='".$r['code_name']."'");
                continue;
            }
            if($r['admin'] == true)
                continue;
    ...
    function fix_inactivity(){
        mysql_query("update agent set inactive=true where admin=false AND kia=false AND last_kill <= NOW() - interval 8 day");
        $q = mysql_query("select tel, carrier, sent_inactivity, code_name, target from agent where inactive=true");
        while($r = mysql_fetch_assoc($q)){
            if(!$r['sent_inactivity']){
                send_sms($r['tel'], $r['carrier'], "You've been inactive for seven days. You are now out of the game.");
                send_sms(get_tel(get_killer($r['code_name'])), get_carrier(get_killer($r['code_name'])), "You've been assigned a new target, " . get_real(get_target(get_killer($r['code_name']))));
                mysql_query("update agent set last_kill=NOW(), target='$r[target]' where code_name='".get_killer($r['code_name']) . "'");
            }
            mysql_query("update agent set sent_inactivity=true, target='' WHERE code_name='$r[code_name]'");
        }
    }
?>
